...
def uv_radtrans(z_upper_limit=64.e5, z_step=1.e5, inputatmofilelabel='volcanicmars_0.02bar_250K_10ppmso2_0h2s', outputfilelabel='_z=0_A=0_noTD_DS_co2cloudod=1000_z=20.5_reff=10_trimSO2', inputspectrafile='general_youngsun_mars_spectral_input.dat', 	h2oiceparamsfile='cloud_h2o_reff10_vareff0p1_lognormal.pickle', co2iceparamsfile='cloud_co2_reff10_vareff0p1_lognormal.pickle', dustparamsfile='dust_wolff_reff1p5_vareff0p5_lognormal.pickle', TDXC=False, DeltaScaling=True, SZA_deg=0., albedoflag='uniformalbedo',uniformalbedo=0., nonuniformalbedo=[0., 1., 0., 0., 0.], includedust=False, tau_d=0.,includeco2cloud=False, co2cloudlayerinds=np.array([43]), co2cloudlayerods=np.array([0.]),includeh2ocloud=False, h2ocloudlayerinds=np.array([60]), h2ocloudlayerods=np.array([0.]), includes8aer=False, tau_s8aer=0., includeh2so4aer=False, tau_h2so4aer=0., s8aerparamsfile='s8_reff0p103_hu2013.pickle', h2so4aerparamsfile='h2so4_215K_reff0p103_hu2013.pickle', H_atm=8.e5):
	...
	--s8aerparamsfile: pick file giving optical properties of S8 aerosols
	--h2so4aerparamsfile: pickle file giving optical properties of H2SO4 aerosols
	
	#Particulate optical depth specification
	---tau_d: total dust optical depth (distributed according to exponential profile)
	
	---tau_s8aer: total S8 Aerosol optical depth (distributed according to exponential profile)
	---tau_h2so4aer: total H2SO4 aerosol optical depth (distributed according to exponential profile)
	---H_atm: atmospheric scale height in cm. N2 atm, mu=28: 8.7e5. CO2-dominated atm, mu=42.4: 5.8e5 cm. (T=288, g=981 cm/s/s)
	...
	---includes8aer: should S8 aerosol be included?
	---includeh2so4aer: should H2SO4 aerosol be included?
	...
	N_particle_species=5 #number of species of particles we are treating, here h2o ice clouds and co2 ice clouds, dust, and s8 and h2so4 aerosols
	...
	##To get S8 aerosol optical depths: assume exponential profile
	s8aer_optical_depths=np.zeros(np.shape(z_center))
	if includes8aer:
		s8aerparamsfile='./ParticulateOpticalParameters/'+s8aerparamsfile
		#H_atm=8.e5 #atm scale height, in cm
		prefactor=tau_s8aer*(np.exp(z_center[-1]/H_atm))*(1.-np.exp(-z_step/H_atm))
		s8aer_optical_depths=prefactor*np.exp(-z_center/H_atm)
	print 'S8 aer optical depth is (unscaled):', np.sum(s8aer_optical_depths), '\n' #check to make sure all shipshape

	##To get H2SO4 aerosol optical depths: assume exponential profile
	h2so4aer_optical_depths=np.zeros(np.shape(z_center))
	if includeh2so4aer:
		h2so4aerparamsfile='./ParticulateOpticalParameters/'+h2so4aerparamsfile
		#H_atm=8.e5 #atm scale height, in cm
		prefactor=tau_h2so4aer*(np.exp(z_center[-1]/H_atm))*(1.-np.exp(-z_step/H_atm))
		h2so4aer_optical_depths=prefactor*np.exp(-z_center/H_atm)
	print 'H2SO4 aer optical depth is (unscaled):', np.sum(h2so4aer_optical_depths), '\n' #check to make sure all shipshape
	#-----------------
	#File I/O
	...
	#NEW
	if includes8aer:
		colden_particles_species_z[3,:]=rts.get_particulate_columndensities(s8aer_optical_depths, s8aerparamsfile) #S8 aerosol, cm**-2
	if includeh2so4aer:
		colden_particles_species_z[4,:]=rts.get_particulate_columndensities(h2so4aer_optical_depths, h2so4aerparamsfile) #H2SO4 aerosol, cm**-2
	...
	#newly added
	if np.sum(colden_particles_species_z[3,:])>0: #only run S8 aerosol optical parameter code if have S8:
		partoptparam_species_wav_z_params[3,:,:,0], partoptparam_species_wav_z_params[3,:,:,1], partoptparam_species_wav_z_params[3,:,:,2]=css.compute_cloud_params(wav_leftedges,wav_rightedges, N_layers, s8aerparamsfile) 
		#pdb.set_trace()

	if np.sum(colden_particles_species_z[4,:])>0: #only run H2SO4 aerosol optical parameter code if have H2SO4:
		partoptparam_species_wav_z_params[4,:,:,0], partoptparam_species_wav_z_params[4,:,:,1], partoptparam_species_wav_z_params[4,:,:,2]=css.compute_cloud_params(wav_leftedges,wav_rightedges, N_layers, h2so4aerparamsfile) 
	...
	print 'S8 column density is (cm-2):', np.sum(colden_particles_species_z[3,:])
	print 'H2SO4 column density is (cm-2):', np.sum(colden_particles_species_z[4,:])
	print '\n'
