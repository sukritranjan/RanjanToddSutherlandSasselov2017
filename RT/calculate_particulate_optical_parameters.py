...
###H2SO4,using data from Hummel+1998, via compendium of Shettle, following treatment by Tian+2010
def indref_h2so4_hummel_300K(wav):
	"""
	This function returns the index of refraction for 75% H2SO4/25% H2O solution, which is commonly used to approximate H2SO4 aerosols, at 300K (room temp)
	
	The data used are cited to Hummel et al. (1988), via the compendium of Shettle, accessed via ftp://cfa-ftp.harvard.edu/pub/HITRAN2012/Aerosols/papers/shettle.dat. This is NOT what Hu et al. (2013) state they used. They state they use the Palmer & Williams (1975) data at UV wavelengths. However, Palmer & Williams only present indices of refraction down to 360 nm; the only include imaginary indices of refraction down to 556 nm. It is therefore extremely unclear how Hu et al. computed their optical parameters at wavelengths down to 100 nm, as shown in Figure 1 of their paper. We instead follow Tian et al. (2010), who Hu et al seem to follow closely for the S-aerosols, in using the Hummel et al data at UV wavelengths.
	
	Input: wavelength in nm
	
	Output: complex index of refraction, calculated using interpolation
	For values outside the range encompassed by the data (i.e. <200 nm or >167 microns) the constant value at 195 nm and 2 um respectively is used.
	"""
	
	refwav_um,N_r, N_i=np.genfromtxt('./Raw_Data/ComplexRefractionIndices/h2so4-300K-shettlecompilation.txt', skip_header=3, skip_footer=0, usecols=(0, 1, 2), unpack=True)#wavelength in microns, real refractive index, imaginary refractive index.

	#index of refraction at 200 nm, for padding
	refwav_um_minval=refwav_um[0]
	N_r_minval=N_r[0]
	N_i_minval=N_i[0]
	
	#index of refraction at 2 um, for padding
	refwav_um_maxval=refwav_um[-1]
	N_r_maxval=N_r[-1]
	N_i_maxval=N_i[-1]
...
def indref_h2so4_hummel_215K(wav):
	"""
	This function returns the index of refraction for 75% H2SO4/25% H2O solution, which is commonly used to approximate H2SO4 aerosols, at 215K (room temp)
	...
	refwav_um,N_r, N_i=np.genfromtxt('./Raw_Data/ComplexRefractionIndices/h2so4-215K-shettlecompilation.txt', skip_header=3, skip_footer=0, usecols=(0, 1, 2), unpack=True)#wavelength in microns, real refractive index, imaginary refractive index.
...
######testing: in particular comparing to Hummel+1998 table 6
#####print indref_h2so4_hummel_300K(200.)
#####print indref_h2so4_hummel_215K(200.)
#####print indref_h2so4_hummel_215K(np.array([190., 200., 225., 250., 275., 300., 330.]))
#####pdb.set_trace()
#######Looks OK!

###S8,using compilation extracted from Tian+2010.
def indref_s8_tian(wav):
	"""
	This function returns the UV indices of refraction for S8 aerosols, via the compulation of Tian et al. (2010). These data are _highly_ uncertain. Imaginary indices of refraction are a combination of data of Sassoon+1985 (0.51-2 um) and Bass+1953 (0.22-0.3 um), while the real indices of refraction are based on an log-linear interpolation between the data of Sasson+1985 at 0.4 um down to an arbitrarily-chosen value of 3 at 0.12 um. So, there is a _lot_ of uncertainty in the UV parameters. Tian et al. (2010) used log-linear interpolation in data gaps. So, there is a *titanic* amount of uncertainty in the optical parameters at UV wavelengths, and they are really just guesses (especially the amount of scattering...). So don't trust these values or results derived from them too much! The Sasson+1985 data are liquid sulfur. The Bass+1953 data are solid S8 dissolved in ethyl alcohol. 
	
	We were unable to obtain data files of these indices from Tian or Claire, so we instead digitized Figure S1 of Tian+2010. We had to digitize the imaginary and real parts separately, and interpolate to a common wavelength scale.  Consequently, these indices cannot be trusted to more than 2 sig figs at best. 
	
	Input: wavelength in nm
	
	Output: complex index of refraction, calculated using interpolation
	For values outside the range encompassed by the data (i.e. <135 nm or >405 nm) the constant value at 195 nm and 2 um respectively is used.
	"""

	refwav_um,N_r, N_i=np.genfromtxt('./Raw_Data/ComplexRefractionIndices/S8_Tian_2010_Indices.txt', skip_header=3, skip_footer=0, usecols=(0, 1, 2), unpack=True)#wavelength in microns, real refractive index, imaginary refractive index.

	
	#index of refraction at 135 nm, for padding
	refwav_um_minval=refwav_um[0]
	N_r_minval=N_r[0]
	N_i_minval=N_i[0]
	
	#index of refraction at 405 nm, for padding
	refwav_um_maxval=refwav_um[-1]
	N_r_maxval=N_r[-1]
	N_i_maxval=N_i[-1]
...
####testing:
###print indref_s8_tian(130.)
###print indref_s8_tian(135.)
###print indref_s8_tian(np.array([135., 200., 225., 250., 275., 300., 330.]))
###pdb.set_trace()
####Looks OK
...
def get_ice_particle_optical_parameters(wav, r_eff, var_eff, indreffunc, dist):
	...
	r_mean=scipy.integrate.quad(num_r, lowerbound, upperbound, args=(r_eff, var_eff), epsabs=0, epsrel=1.e-6)[0]/G
	print 'r_mean', r_mean #This is a check, if it doensn't match we have some problems...
	...
	for ind in range(0, len(wav)):
		...
		qsca_mean[ind]=scipy.integrate.quad(num_qsca, lowerbound, upperbound, args=(r_eff, var_eff, wavelength, n_particle), epsabs=0, epsrel=1.e-1, limit=500)[0]/G
		w_0_mean[ind]=scipy.integrate.quad(num_w_0, lowerbound, upperbound, args=(r_eff, var_eff, wavelength, n_particle), epsabs=0, epsrel=1.e-1, limit=500)[0]/G
		g_mean[ind]=scipy.integrate.quad(num_g, lowerbound, upperbound, args=(r_eff, var_eff, wavelength, n_particle), epsabs=0, epsrel=1.e-1, limit=500)[0]/G
...
def get_ice_particle_optical_parameters_logged(wav, r_eff, var_eff, indreffunc, dist):
	"""
	Compute the average values using numerical quadrature, and weighting by n(r)*pi*r**2, as shown for Q_sca in equation 2.58 of Hansen and Travis. Uses LOG SPACING for numerical efficiency.
	"""
	#indreffunc=indref_water_ice_warren_2008
	#dist=modified_gamma #set distribution to use
	
	def denom(logr, r_eff, var_eff):
		r=np.exp(logr)
		return np.pi*r**2.*dist(r, r_eff, var_eff)
	def num_r(logr, r_eff, var_eff):
		r=np.exp(logr)
		return r*np.pi*r**2.*dist(r, r_eff, var_eff)
	def num_qsca(logr, r_eff, var_eff, wavelength, n_particle):
		r=np.exp(logr)
		return get_optical_parameters_mie_ppc(r, wavelength, n_medium, n_particle, nterm, 'qsca')*np.pi*r**2.*dist(r, r_eff, var_eff)
	def num_w_0(logr, r_eff, var_eff,wavelength, n_particle):
		r=np.exp(logr)
		return get_optical_parameters_mie_ppc(r, wavelength, n_medium, n_particle, nterm, 'w_0')*np.pi*r**2.*dist(r, r_eff, var_eff)
	def num_g(logr, r_eff, var_eff, wavelength, n_particle):
		r=np.exp(logr)
		return get_optical_parameters_mie_ppc(r, wavelength, n_medium, n_particle, nterm, 'g')*np.pi*r**2.*dist(r, r_eff, var_eff)
	...
	lowerbound=np.log(r_eff*10.**(-10.*var_eff)) #this is a very arbitrary formulation for the lower bound...
	upperbound=np.log(r_eff*10.**(4.*var_eff)) #also super arbitrary for the lower bound...
	...
	for ind in range(0, len(wav)):
		...
		qsca_mean[ind]=scipy.integrate.quad(num_qsca, lowerbound, upperbound, args=(r_eff, var_eff, wavelength, n_particle), epsabs=0, epsrel=1.e-2, limit=1000)[0]/G
		w_0_mean[ind]=scipy.integrate.quad(num_w_0, lowerbound, upperbound, args=(r_eff, var_eff, wavelength, n_particle), epsabs=0, epsrel=1.e-2, limit=1000)[0]/G
		g_mean[ind]=scipy.integrate.quad(num_g, lowerbound, upperbound, args=(r_eff, var_eff, wavelength, n_particle), epsabs=0, epsrel=1.e-2, limit=1000)[0]/G
...
#########################
####Generate output (radii in um, wavelengths in nm)
#########################

wav=np.arange(190., 511., step=5.)

##Reproduce Tian+2010 Fig S1
#generate_cloud_optical_params(wav, 0.01, 0.001, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p01_vareff0p001_lognormal')
#generate_cloud_optical_params(wav, 0.5, 0.001, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p5_vareff0p001_lognormal')
#generate_cloud_optical_params(wav, 1., 0.001, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff1_vareff0p001_lognormal')

###Reproduce Tian+2010 Fig S2
#generate_cloud_optical_params(wav, 0.2, 0.001, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff0p2_vareff0p001_lognormal')

###Check ourselves against some of Meg Smith's calculations, kindly shared with us by Mark Claire. Her's were monodisperse, but a rigid calculation should be good enough.
#meg_wav=np.array([256.3, 280.0, 296.1, 318.2, 335.2, 365.6, 420.0, 495.0])
#generate_cloud_optical_params(meg_wav, 0.001, 0.001, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p001_vareff0p001_lognormal_meg')
#generate_cloud_optical_params(meg_wav, 0.1, 0.001, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p1_vareff0p001_lognormal_meg')
#generate_cloud_optical_params(meg_wav, 1., 0.001, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff1_vareff0p001_lognormal_meg')

##reproduce (as best we can) Hu+2013 stuff

#generate_cloud_optical_params(wav, 0.05, 0.7, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p05_hu2013')
#generate_cloud_optical_params(wav, 0.05, 0.7, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff0p05_hu2013')
generate_cloud_optical_params(wav, 0.103, 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p103_hu2013')
generate_cloud_optical_params(wav, 0.103, 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff0p103_hu2013')
#generate_cloud_optical_params(wav, 0.2, 0.7, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p2_hu2013')
#generate_cloud_optical_params(wav, 0.2, 0.7, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff0p2_hu2013')


#generate_cloud_optical_params(wav, 0.5, 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff0p5_hu2013')
#generate_cloud_optical_params(wav, 0.5, 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff0p5_hu2013')
#generate_cloud_optical_params(wav, 5., 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff5_hu2013')
#generate_cloud_optical_params(wav, 5., 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff5_hu2013')

#generate_cloud_optical_params(wav, 1., 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff1_hu2013')
#generate_cloud_optical_params(wav, 1., 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff1_hu2013')
#generate_cloud_optical_params(wav, 10., 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff10_hu2013')
#generate_cloud_optical_params(wav, 10., 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff10_hu2013')

#generate_cloud_optical_params(wav, 2., 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff2_hu2013')
#generate_cloud_optical_params(wav, 2., 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff2_hu2013')
#generate_cloud_optical_params(wav, 20., 0.62, indref_s8_tian, lognormal, './ParticulateOpticalParameters/s8_reff20_hu2013')
#generate_cloud_optical_params(wav, 20., 0.62, indref_h2so4_hummel_215K, lognormal, './ParticulateOpticalParameters/h2so4_215K_reff20_hu2013')
