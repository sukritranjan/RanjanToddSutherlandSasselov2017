...
if atmtype=='co2rich':
	mr_h2s_list=np.array([4.e-10, 1.e-9, 9.e-9, 5.e-8, 2.e-7, 7.e-7, 2.e-6, 9.e-6]) #column-integrated mixing ratios
	mr_so2_list=np.array([3.e-10, 9.e-10, 3.e-9, 7.e-9, 1.e-8, 3.e-8, 8.e-8, 3.e-7]) #column-integrated mixing ratios
elif atmtype=='n2rich':
	mr_h2s_list=np.array([9.e-14, 9.e-14, 2.e-13, 7.e-13, 4.e-10, 1.e-7, 2.e-6, 7.e-6]) #column-integrated mixing ratios; first 2 are upper bounds
	mr_so2_list=np.array([4.e-12, 1.e-11, 5.e-11, 2.e-10, 2.e-9, 2.e-8, 8.e-8, 2.e-7]) #column-integrated mixing ratios
else:
...
def activity_coefficient(species, I):
	...
	if species=='H+':
		...
		B=.3258E-8
		z=1.0
		coeff=10**(-A*z*z*np.sqrt(I))/(1+ai*B*np.sqrt(I))
		if I>0.1:
			coeff+=0.2*I
	elif species=='OH-':
		ai=3.5E8
...
if run_ph2s_pso2_chem:
	...
	I_list=np.array([0., 0.1, 1.]) #List of ionic strengths to consider: 0 (no ions), 0.1 (approx limit from vesicle formation), and 1 (approx modern oceans, at 0.7). Upper bound on salinity comes from saturation of water; dead sea at ~7 is approx at this limit.
	...
	for ax1ind in range(0, 2):
		for ax2ind in range(0,2):
			...
			ax[ax1ind, ax2ind].axvspan(1.e-4, 1., color='red', alpha=0.2) #Ranjan+2016b photochemical-quenching level
	...
	plt.savefig('./Plots/h2s_speciation_paper.pdf', orientation='portrait',papertype='letter', format='pdf')
	...
	for ax1ind in range(0, 3):
		for ax2ind in range(0,2):
			...
			ax[ax1ind, ax2ind].axvspan(1.e-5, 1., color='red', alpha=0.2) #Ranjan+2016b photochemical-quenching level
	...
	plt.savefig('./Plots/so2_speciation_paper.pdf', orientation='portrait',papertype='letter', format='pdf')
...
if run_s_flux_chem:
	...
	ax[0].axvspan(1.*10.**(10.), 1.*10.**(11.5), color='grey', alpha=0.2) #Max S emission, during basaltic plain emplacement, fro Halevy et al 2014 
	...
	ax[1].axvspan(1.*10.**(10.), 1.*10.**(11.5), color='grey', alpha=0.2) #Max S emission, during basaltic plain emplacement, fro Halevy et al 2014 
	...
	ax[2].axvspan(1.*10.**(10.), 1.*10.**(11.5), color='grey', alpha=0.2) #Max S emission, during basaltic plain emplacement, fro Halevy et al 2014 
	ax[2].legend(loc='upper left', ncol=1, borderaxespad=0., fontsize=12)

	fig.subplots_adjust(wspace=0., hspace=0.1)
	plt.savefig('./Plots/h2s_so2_speciation_phis_'+atmtype+'_paper.pdf', orientation='portrait',papertype='letter', format='pdf')
...
if run_rt_plot:
	...
	########################
	###Plot surface radiance for phi_s's without aerosols but with full atmosphere approximation
	########################
	fig5, ax=plt.subplots(1, figsize=(8., 6.), sharex=True, sharey=True)
	markersizeval=5.
	...
	for ind in range(0, len(phi_s_list[:-3])):
		#pdb.set_trace()
		elt=elt_list[ind]
		twostr_wav, twostr_toa, twostr_surfint=np.genfromtxt('./RT/TwoStreamOutput/hu2013_'+atmtype+'_sflux'+elt+'_exponential_no_aerosols.dat', skip_header=1, skip_footer=0,usecols=(2,3,6), unpack=True)
	...
	plt.savefig('./Plots/h2s_so2_radiance_twostr_noaer_'+atmtype+'.pdf', orientation='portrait',papertype='letter', format='pdf')


	########################
	###FINALLY, plot surface radiance for phi_s's with aerosols and full atm
	########################
	fig6, ax=plt.subplots(1, figsize=(8., 6.), sharex=True, sharey=True)
	markersizeval=5.
	colors=cm.rainbow(np.linspace(0,1,len(phi_s_list[:-2])))
	...
	ax.tick_params(axis='y', labelsize=18)	
	ax.tick_params(axis='x', labelsize=18)
	plt.savefig('./Plots/h2s_so2_radiance_twostr_aer_'+atmtype+'.pdf', orientation='portrait',papertype='letter', format='pdf')
	

	########################
	###Dose rate plot (just 200-300 nm radiation, pathway-independent measure)
	########################
	###Read in computed doses
	elt_list=np.array(['3e9', '1e10', '3e10', '1e11', '3e11', '1e12'])
	elt_axis=np.array([3.e9, 1.e10, 3.e10, 1.e11, 3.e11, 1e12])
	
	num_elt=len(elt_list)
	
	dose_200_300_aer=np.zeros([num_elt]) #surface radiance integrated 200-300 nm, aerosol case
	dose_200_300_noaer=np.zeros([num_elt]) #surface radiance integrated 200-300 nm, NO aerosol case

	
	for ind in range(0, num_elt):
		elt=elt_list[ind]
		
		dose_200_300_aer[ind]=np.genfromtxt('./RT/DoseRates/dose_rates_hu2013_'+atmtype+'_sflux'+elt+'_exponential_aerosols.dat', skip_header=1, skip_footer=0,usecols=(1), unpack=True)
		dose_200_300_noaer[ind]=np.genfromtxt('./RT/DoseRates/dose_rates_hu2013_'+atmtype+'_sflux'+elt+'_exponential_no_aerosols.dat', skip_header=1, skip_footer=0,usecols=(1), unpack=True)

	###Plot results

	fig, ax=plt.subplots(1, figsize=(8., 6.), sharex=True, sharey=False)
	markersizeval=3.
	
	#ax.set_title('')
	ax.plot(elt_axis, dose_200_300_aer, marker='s', markeredgewidth=0., markersize=markersizeval, linewidth=1, color='black', label=r'Aerosol')
	ax.plot(elt_axis, dose_200_300_noaer, marker='s', markeredgewidth=0., markersize=markersizeval, linewidth=1, color='red', label=r'No Aerosol')
	
	#ax.axvline(254., linestyle='--', color='black')
	ax.legend(loc=0, ncol=2, borderaxespad=0., fontsize=14)
	ax.set_xlabel(r'$\phi_{S}$ (cm$^{-2}$ s$^{-1}$)', fontsize=18)
	ax.set_xscale('log')
	ax.set_xlim([3.e9,1.e12])
	ax.set_ylabel(r'200-300 nm Relative Dose Rate', fontsize=18)
	ax.set_yscale('log')
	ax.set_ylim([1.e-3, 2.])
	ax.tick_params(axis='y', labelsize=18)	
	ax.tick_params(axis='x', labelsize=18)
	plt.savefig('./Plots/h2s_so2_doserates'+atmtype+'.pdf', orientation='portrait',papertype='letter', format='pdf')
